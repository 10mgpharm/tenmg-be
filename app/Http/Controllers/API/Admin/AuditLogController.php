...
class AuditLogController extends Controller
{
    ...
    public function index(SearchAuditLogRequest $request): JsonResponse
    {
        $query = Activity::query()
            ...
            ->when(
                $request->input('crud_type'),
                fn($query, $crud_type) => $query->whereIn(
                    'properties->crud_type',
                    is_array($crud_type)
                        ? array_unique(array_merge(...array_map(fn($s) => trim($s), $crud_type)))
                        : array_unique(array_merge(...array_map(fn($s) => trim($s), explode(",", $crud_type))))
                )
            )
            ->when(
                $request->input('ip'),
                fn($query, $ip) => $query->whereIn(
                    'properties->ip',
                    is_array($ip)
                        ? array_unique(array_merge(...array_map(fn($s) => trim($s), $ip)))
                        : array_unique(array_merge(...array_map(fn($s) => trim($s), explode(",", $ip))))
                )
            )
            ->when(
